...
from utils.constants import SUPPORT_CARD_ICON_REGION, MOOD_REGION, TURN_REGION, FAILURE_REGION, YEAR_REGION, MOOD_LIST, CRITERIA_REGION, EVENT_NAME_REGION, ENERGY_BAR_REGION
...
# Estimate energy bar fill percentage
def check_energy(region=ENERGY_BAR_REGION, image=None):
  if image is None:
  ...
  column_mask = colored_mask.mean(axis=0) > 0.5
  percent = int((column_mask.sum() / column_mask.size) * 100)
  return percent

# Testing helper to fine-tune energy bar region
def test_energy_scan(region=ENERGY_BAR_REGION, save_path="energy_test.png"):
  img = capture_region(region)
  img.save(save_path)
  percent = check_energy(region, image=img)
  print(f"[INFO] Energy bar approx {percent}% full. Saved debug image to {save_path}")
  return percent
